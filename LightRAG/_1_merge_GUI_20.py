...
def fetch_entity_details(label):
    try:
        ...
        messagebox.showerror("Error", f"Failed to fetch entity details for {label}: {e}")
        return f"Error: {e}", "", ""
...
class MergeGUI:
    ...
    def create_left_panel(self):
        ...
        self.left_panel.grid_rowconfigure(2, weight=1) # The list frame should expand
        self.left_panel.grid_columnconfigure(0, weight=1)
        
        # Filter and action frame at the top of the left panel
        top_controls_frame = ttk.Frame(self.left_panel)
        top_controls_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        top_controls_frame.grid_columnconfigure(1, weight=1) # Filter entry expands
        ...
        # New: Action buttons for selection management
        ...
        # Main content frame for scrollable list
    ...
    def create_entity_list(self):
        ...
        # Clear the mapping for currently visible checkboxes
        ...
        for i, ent in enumerate(self.filtered_entity_list):
            ...
            if var is None: # Should not happen if entity_list is consistent with all_check_vars
                var = tk.BooleanVar()
    ...
    def update_selection(self):
        ...
        # Clear target and entity type if current values are no longer in selected list
        ...
        # This check is implicitly done in create_entity_list for filtered list.
        # Here we ensure it's valid against all selected if radio buttons are used.
    ...
    def calculate_tile_layout(self, available_width, available_height, num_items, min_height=200):
        ...
        # Desired width for each description frame is approximately half the available width
        effective_min_width = max(300, (available_width // 2) - 10) 
        
        cols = max(1, available_width // effective_min_width)
            
        # Ensure we don't have too many columns if there are few items, or not enough space for 2 columns.
        ...
        cols = min(cols, num_items) # Don't create more columns than items
    ...
    def toggle_descriptions(self):
        ...
        if any_open:
            ...
            # Show descriptions only for currently selected items
            selected = [label for label, var in self.all_check_vars.items() if var.get()]
            if not selected:
            ...
            # Get current size of the scrollable frame for descriptions
            ...
            # Ensure the scrollable frame can accommodate the grid
            for c in range(cols):
    ...
    async def run_merge(self, sources, target, strategy, etype):
        ...
        try:
            ...
            # --- Attempt to refresh the LightRAG server ---
            ...
            # This will also recreate `self.all_check_vars` with new entities/removed ones
            self.entity_list = fetch_entities()
            # Re-initialize all_check_vars to include newly fetched entities, preserving existing states
            new_all_check_vars = {}
            for entity in self.entity_list:
                new_all_check_vars[entity] = self.all_check_vars.get(entity, tk.BooleanVar())
            self.all_check_vars = new_all_check_vars
            
            self.filter_var.set("") # Clear filter before refreshing list
            self.filtered_entity_list = self.entity_list.copy()
            self.create_entity_list() # Rebuild the visible list
