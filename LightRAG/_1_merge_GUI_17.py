...
class MergeGUI:
    ...
    def load_window_config(self):
        ...
        self.window_config = {
            "geometry": "1200x800",
            "state": "normal",
            "paned_position": 600  # Changed from 800 to 600 for better center positioning
        }
    ...
    def set_initial_paned_position(self):
        """Set the initial position of the paned window splitter"""
        try:
            # Get the current window width
            self.root.update_idletasks()
            window_width = self.paned_window.winfo_width()
            
            # If we have a saved position and it's reasonable, use it
            # Otherwise, default to 60% of window width for the left panel
            if (hasattr(self, 'window_config') and 
                'paned_position' in self.window_config and 
                window_width > 100):  # Make sure window is properly sized
                
                saved_pos = self.window_config["paned_position"]
                # Ensure the saved position is reasonable (between 30% and 80% of window width)
                min_pos = int(window_width * 0.3)
                max_pos = int(window_width * 0.8)
                
                if min_pos <= saved_pos <= max_pos:
                    position = saved_pos
                else:
                    position = int(window_width * 0.6)  # Default to 60%
            else:
                position = int(window_width * 0.6) if window_width > 100 else 600
    ...
    def create_ui(self):
        ...
        # Set initial paned window position - use a more reliable method
