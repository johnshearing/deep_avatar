...
class MergeGUI:
    ...
    def calculate_tile_layout(self, available_width, available_height, num_items, min_height=200):
        """Calculate optimal tiling layout for description frames, making each frame half the width."""
        if num_items == 0:
            return 0, 0, 0, 0
        ...
        effective_min_width = max(300, available_width // 2 - 10) # Subtract 10 for padding/margins
        
        # Calculate number of columns that can fit
        cols = max(1, available_width // effective_min_width)
        
        # If only one item and it's less than half the width, still try to make it half
        if num_items == 1 and cols == 1 and available_width > 600: # Assuming a large enough window
            cols = 2 # Try to force 2 columns even for 1 item if there's enough space for 2 half-width windows
            
        # Ensure we don't have too many columns if there are few items
    ...
    def toggle_descriptions(self):
        ...
        if any_open:
            ...
            # If the scrollable frame hasn't rendered yet, use a reasonable default.
            # This is a fallback and ideally, winfo_width() should give a proper value.
            if available_width < 100:
                available_width = self.right_panel.winfo_width() - 20 # Estimate based on right panel
                if available_width < 100: available_width = 800 # Absolute fallback
            ...
            # Calculate layout (note: min_width argument removed, it's now internal logic)
            ...
            for idx, label in enumerate(selected):
                try:
                    ...
                    # Use the calculated frame_width and frame_height
                    desc_frame.config(width=frame_width - 4, height=frame_height - 4) # Adjust for padding
