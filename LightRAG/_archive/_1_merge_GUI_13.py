...
class MergeGUI:
    ...
    def create_entity_list(self):
        ...
        # Create a frame inside the radio frame to control spacing
        radio_inner_frame = ttk.Frame(self.radio_frame)
        radio_inner_frame.pack(fill="both", expand=True)
        
        # Create entity rows with matching heights
        for i, ent in enumerate(self.entity_list):
            ...
            # Corresponding radio button in the radio frame with matching spacing
            rb = ttk.Radiobutton(radio_inner_frame, text="", variable=self.first_entity_var, value=ent)
            rb.pack(pady=1)  # Match the checkbox padding exactly
