    ...
    try:
        ...
        for node in data.get("nodes", []):
            if node.get("id") == label:
                desc = node["properties"].get("description", "No description found.")
                entity_type = node["properties"].get("entity_type", "")
                source_id = node["properties"].get("source_id", "")
                file_path = node["properties"].get("file_path", "") # Added file_path
                return str(desc), str(entity_type), str(source_id), str(file_path) # Return file_path
        return "No description found.", "", "", "" # Default values including empty file_path
    except requests.exceptions.ConnectionError:
        print("Connection Error: Could not connect to LightRAG server to fetch entity details.")
        return "Error: Server not reachable.", "", "", "" # Default values including empty file_path
    except Exception as e:
        print(f"Error fetching entity details for {label}: {e}")
        return f"Error: {e}", "", "", "" # Default values including empty file_path
...
class MergeGUI:
    def __init__(self, root):
        ...
        self.entity_data = {} # Cache for entity details (description, type, source_id, file_path)
    ...
    def select_all_entities(self):
        for label, var in self.all_check_vars.items():
            ...
            if label not in self.entity_data:
                ...
                desc, typ, srcid, fpath = fetch_entity_details(label)
                # Updated to store 'fpath'
                self.entity_data[label] = {"desc": desc, "type": typ, "srcid": srcid, "fpath": fpath}
    ...
    def create_right_panel(self):
        ...
        self.desc_canvas.configure(yscrollcommand=self.desc_scrollbar.set) # Corrected line
    ...
    def update_selection(self):
        ...
        for label in selected:
            if label not in self.entity_data:
                # Unpack 4 values: desc, typ, srcid, fpath
                desc, typ, srcid, fpath = fetch_entity_details(label)
                # Store all 4 values
                self.entity_data[label] = {"desc": desc, "type": typ, "srcid": srcid, "fpath": fpath}
    ...
    def toggle_descriptions(self):
        ...
        if any_open:
            ...
            for idx, label in enumerate(selected):
                try:
                    if label not in self.entity_data:
                        # Unpack 4 values: desc, entity_type, srcid, fpath
                        desc, entity_type, srcid, fpath = fetch_entity_details(label)
                        # Store all 4 values
                        self.entity_data[label] = {"desc": desc, "type": entity_type, "srcid": srcid, "fpath": fpath}
                    else:
                        # Retrieve all 4 values from cache
                        desc = self.entity_data[label].get("desc", "No description found.")
                        entity_type = self.entity_data[label].get("type", "No type found.")
                        srcid = self.entity_data[label].get("srcid", "")
                        fpath = self.entity_data[label].get("fpath", "")
