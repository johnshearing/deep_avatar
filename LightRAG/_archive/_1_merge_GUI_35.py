...
class MergeGUI:
    ...
    def create_right_panel(self):
        ...
        # Merge Controls Frame
        merge_controls_frame = ttk.LabelFrame(self.right_panel, text="Merge Controls", padding=10)
        merge_controls_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=10)
        merge_controls_frame.grid_columnconfigure(1, weight=1)

        ttk.Label(merge_controls_frame, text="Target Entity:").grid(row=0, column=0, sticky="w", pady=2)
        self.target_entry = ttk.Combobox(merge_controls_frame, state="readonly")
        self.target_entry.grid(row=0, column=1, sticky="ew", padx=5, pady=2)

        ttk.Label(merge_controls_frame, text="Entity Type:").grid(row=1, column=0, sticky="w", pady=2)
        self.entity_type = ttk.Combobox(merge_controls_frame, state="readonly")
        ...
        strategy_frame = ttk.LabelFrame(merge_controls_frame, text="Merge Strategy", padding=10)
        ...
        ttk.Radiobutton(strategy_frame, text="Append", variable=self.strategy_desc, value="append").grid(row=0, column=1, sticky="w")
        ttk.Radiobutton(strategy_frame, text="Keep First", variable=self.strategy_desc, value="keep_first").grid(row=0, column=2, sticky="w")

        # Source ID Strategy
        ttk.Label(strategy_frame, text="Source ID:").grid(row=1, column=0, sticky="w", pady=2)
        self.strategy_srcid = tk.StringVar(value="union")
        ttk.Radiobutton(strategy_frame, text="Union", variable=self.strategy_srcid, value="union").grid(row=1, column=1, sticky="w")
        ttk.Radiobutton(strategy_frame, text="Keep First", variable=self.strategy_srcid, value="keep_first").grid(row=1, column=2, sticky="w")

        # Action Buttons
        button_frame = ttk.Frame(merge_controls_frame)
        ...
        description_display_frame = ttk.LabelFrame(self.right_panel, text="Entity Descriptions", padding=10)
        description_display_frame.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
        description_display_frame.grid_rowconfigure(0, weight=1)
        description_display_frame.grid_columnconfigure(0, weight=1)

        self.desc_canvas = tk.Canvas(description_display_frame)
        self.desc_scrollbar = ttk.Scrollbar(description_display_frame, orient="vertical", command=self.desc_canvas.yview)
    ...
    def create_entity_list(self):
        ...
        for idx, entity in enumerate(self.filtered_entity_list):
            ...
            # Show/Hide Description button for each entity
            ...
            radio_button.grid(row=idx, column=1, sticky="w", padx=(0, 5), pady=1)

            # Checkbox for selection
            cb = ttk.Checkbutton(self.scrollable_frame, text=entity, variable=var,
                                 command=self.update_selection)
            cb.grid(row=idx, column=2, sticky="ew", padx=(0, 5), pady=1)
