    ...
    try:
        ...
        for node in data.get("nodes", []):
            if node.get("id") == label:
                ...
                return str(desc), str(entity_type), str(source_id)
...
class MergeGUI:
        ...
        if any_open:
            ...
            for idx, label in enumerate(selected):
                try:
                    ...
                    text_widget = tk.Text(text_frame, wrap="word", font=("TkDefaultFont", 9), height=5) # Added height
    ...
    async def run_delete(self):
        ...
        try:
            ...
            if success_count > 0:
                messagebox.showinfo("Deletion Complete", f"Successfully deleted {success_count} entities. Failed to delete {fail_count} entities.")
                if failed_entities:
