...
API_KEY = os.getenv("EMBEDDING_BINDING_API_key") # Corrected potential typo here if it was 'key' before
...
# --- UPDATED: Function to update entity description via API using the provided curl command ---
def update_entity_description_api(entity_label, new_description):
    """
    Sends a request to the LightRAG server to update an entity's description
    using the provided /graph/entity/edit endpoint.
    """
    try:
        ...
        payload = {
            ...
            "allow_rename": False # As per your curl, we're not allowing rename here
        }
...
class MergeGUI:
    def __init__(self, root):
        ...
        self.description_windows = {} # This is not directly used for frames, but can be for separate popups
        self.description_frames = {} # This stores the ttk.LabelFrame widgets
    ...
    def create_left_panel(self):
        ...
        # Adjust columnconfigure for the new button
        ...
        # --- FIX: Removed logic here that was unintentionally preventing descriptions from opening ---
        # The logic to decide to show/hide should be within toggle_descriptions based on self.description_frames
        self.toggle_desc_button = ttk.Button(content_frame, text="Show Descriptions", command=self.toggle_descriptions, width=15) # Changed text for clarity
    ...
    def select_all_entities(self):
        """Selects all entities in the full entity list, regardless of filter, and populates cache."""
        for label, var in self.all_check_vars.items():
            var.set(True)
            # Ensure entity details are fetched and cached for ALL entities when Select All is pressed
        ...
        self.filter_var.set("") # Clear any filter to show all entities
        self.filtered_entity_list = self.entity_list.copy()
        self.create_entity_list() # This will call update_selection

    def select_all_of_entity_type(self):
        """Selects all entities that match the currently selected entity type in the dropdown."""
        ...
        # First, unselect all entities
        for var in self.all_check_vars.values():
            var.set(False)
        
        # Then, select only those that match the chosen type
        entities_of_selected_type = []
        for label in self.entity_list: # Iterate through the full list of entities
            # Ensure entity details are fetched and cached for this entity
            if label not in self.entity_data:
                desc, typ, srcid = fetch_entity_details(label)
                self.entity_data[label] = {"desc": desc, "type": typ, "srcid": srcid}
            
            # Check if the entity's type matches the selected type
            if self.entity_data[label]["type"] == selected_type:
                self.all_check_vars[label].set(True)
                entities_of_selected_type.append(label)

        # Clear any existing filter and update the displayed list
        self.filter_var.set("")
        # For this function, we want to show all the entities of the selected type,
        # so we effectively set the filtered list to these entities.
        self.filtered_entity_list = sorted(entities_of_selected_type, key=lambda x: x.lower())
        self.create_entity_list() # This will rebuild the list with updated checkboxes and call update_selection
    ...
    def update_selection(self):
        ...
        for label in selected:
            # Crucial: Ensure entity data is fetched and cached *before* attempting to retrieve its type
            if label not in self.entity_data:
                desc, typ, srcid = fetch_entity_details(label)
                self.entity_data[label] = {"desc": desc, "type": typ, "srcid": srcid}
    ...
    def toggle_descriptions(self):
        ...
        if any_open:
            # If descriptions are currently open, close them
            for frame in self.description_frames.values():
                frame.destroy()
            self.description_frames.clear()
            self.toggle_desc_button["text"] = "Show Descriptions" # Reset button text
        else:
            # If descriptions are closed, open them for selected entities
            selected = [label for label, var in self.all_check_vars.items() if var.get()]
            if not selected:
            ...
            # Ensure the scrollable frame is correctly sized before calculating layout
            ...
            # Fallback for initial sizing issues
            ...
            for idx, label in enumerate(selected):
                try:
                    # Ensure entity data is fetched and cached here too, before displaying
                    if label not in self.entity_data:
                        desc, typ, srcid = fetch_entity_details(label)
                        self.entity_data[label] = {"desc": desc, "type": typ, "srcid": srcid}
                    ...
                    desc_frame = ttk.LabelFrame(self.desc_scrollable_frame, text="", padding=5) # LabelFrame text is now empty
                    ...
                    # Header for the frame with entity name and edit button
                    header_sub_frame = ttk.Frame(desc_frame)
                    header_sub_frame.pack(fill="x")
                    header_sub_frame.grid_columnconfigure(0, weight=1) # Entity Name Label
                    header_sub_frame.grid_columnconfigure(1, weight=0) # Edit Button
                    ...
                    text_frame.pack(fill="both", expand=True, pady=(5,0)) # Added pady for spacing
                    ...
                    # Display description and source ID as read-only
            ...
            # Change button text to reflect that descriptions are now shown
    ...
    def open_edit_description_modal(self, entity_label):
        """Opens a modal window to edit the description of a specific entity."""
        modal = tk.Toplevel(self.root)
        modal.title(f"Edit Description for {entity_label}")
        modal.transient(self.root) # Make it transient to the main window
        modal.grab_set() # Make it modal
        modal.protocol("WM_DELETE_WINDOW", modal.destroy) # Allow closing with X button
        ...
        # Center the modal window relative to the root window (optional, but good UX)
        self.root.update_idletasks()
        modal.update_idletasks()
        # Set a default size for the modal if it's too small, before centering
        min_width = 400
        min_height = 300
        ...
        self.root.wait_window(modal) # Wait until the modal is closed

    def save_entity_description(self, entity_label, description_text_widget, modal_window):
        """Saves the new description via API and updates the UI."""
        ...
        if update_entity_description_api(entity_label, new_description):
            ...
            # Refresh the descriptions view to show the updated text
            # Ensure descriptions are currently visible before attempting to refresh
            if self.description_frames:
                # Destroy existing description frames to clear the old text
                for frame in list(self.description_frames.values()): # Iterate over a copy as dict changes during loop
                    frame.destroy()
                self.description_frames.clear()
                # Re-open them. This will fetch from the updated cache.
                self.toggle_descriptions() # This re-draws descriptions from the updated cache
            
            modal_window.destroy()
            # message box is already shown by update_entity_description_api
        # else: error message already shown by update_entity_description_api
    ...
    def submit_merge(self):
        ...
        # Close any open description windows/frames before merging
