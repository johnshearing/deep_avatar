...
class MergeGUI:
    ...
    def create_left_panel(self):
        ...
        header_frame.grid_columnconfigure(0, weight=1) # For the Show/Hide Desc label
        header_frame.grid_columnconfigure(1, weight=0) # For "First"
        header_frame.grid_columnconfigure(2, weight=1) # For "Select Entities"
        
        # --- MODIFICATION: Moved and formatted "Show/Hide Desc" label ---
        ...
        ttk.Label(header_frame, text="First", font=("TkDefaultFont", 9, "bold")).grid(row=0, column=1, padx=5, sticky="w")
        ...
        # --- MODIFICATION: Simplified "Show Descriptions" button ---
        self.toggle_desc_button = ttk.Button(content_frame, text="⇕", command=self.toggle_descriptions, width=3) # Use a compact symbol
        ...
        # The first column is for the "Show/Hide Desc" button column and the radio button, 
        # so it should be tight. The second column is for the checkbox text and needs to expand.
        self.scrollable_frame.grid_columnconfigure(0, weight=0) # For radio button
        self.scrollable_frame.grid_columnconfigure(1, weight=1) # For checkbox text (entity label)
    ...
    def create_right_panel(self):
        ...
        # Configure columns for controls. Labels on the left, comboboxes on the right.
        control_frame.grid_columnconfigure(0, weight=0) # For labels (fixed width)
        control_frame.grid_columnconfigure(1, weight=1) # For entry/comboboxes (expands)
        ...
        # --- MODIFICATION: Moved Merge Entities button to the left ---
        self.merge_button = ttk.Button(control_frame, text="Merge Entities", command=self.submit_merge)
        self.merge_button.grid(row=5, column=0, sticky="w", padx=5, pady=10) # Moved to column 0, sticky="w"
    ...
    def toggle_descriptions(self):
        ...
        if any_open:
            for frame in list(self.description_frames.values()): # Iterate over copy
                frame.destroy()
            self.description_frames.clear()
            # --- MODIFICATION: Update header label, not button text ---
