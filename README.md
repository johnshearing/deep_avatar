...
2. Recently, I built the app shown below, which makes it possible to rapidly examine and clean any dirty data that gets ingested into the knowledge graph and vector database during the indexing process.  
   - Currently I am porting this app to the LightRAG WebUI so that only one single application is necessary to create and maintain the data and so that this application is available online to all with the correct privileges.  
<br>
<p>
...
3. Collect and sanitize all training data so we are starting with good information about Charles and Cardano.   
